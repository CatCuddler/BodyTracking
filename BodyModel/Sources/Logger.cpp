...
void Logger::startEvaluationLogger(const char* filename) {
	...
	// Append header
	evaluationDataOutputFile << "Time NodeID Pose identifiedPose Trials Head Hip LeftArm RightArm LeftLeg RightLeg\n";
...
void Logger::saveEvaluationData(float lastTime, int nodeID, int pose, int poseIdentified, int trials, bool head, bool hip, bool left_arm, bool right_arm, bool left_leg, bool right_leg) {
	evaluationDataOutputFile << lastTime << " " << nodeID << " " << pose << " " << poseIdentified << " " << trials << " " << head << " " << hip << " " << left_arm << " " << right_arm << " " << left_leg << " " << right_leg << "\n";
	evaluationDataOutputFile.flush();
}
