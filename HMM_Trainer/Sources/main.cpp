...
// Content: automatically being executed as main script of the program; calls all relevant other scripts and calculates a new HMM either based on user input or variables introduced below. Can alternatively be used to calculate the probability of a new dataset based on existing HMMs by using the corresponding variabels below.
...
const bool useRotation = true;
// Create new HMM based on the all the training file
const bool createHMM = false;
// Calculate new HMMs and replace the old ones if those are better, only end when hmmtries reach max value
const bool optimiseInfiniteHMM = false;
// Try all the combination of parameters (include numStates, numEmissions, lrDepths, tracker files)
// Outputting table of probabilities in overview file.
const bool optimiseMovementRecognition = false;
// Calculating the probability for a dataset based on an already existing HMM.
const bool recognitionTest = false;
...
const string validationFileName = "Yoga_Krieger_";
...
int main() {
	...
	if (createHMM) {
		...
		vector<KMeans> kmeans = calculateClusters(0, trainingNumber, numEmissions, 1000);
	...
	// Traverse all the possible parameter combination and output the corresponding probabilities, this function is help to find out which parameters have better result.
		...
		for (int &numEmissions : emissionIterations) {
			EmissionIndex++;
			kmeans = calculateClusters(0, trainingNumber, numEmissions, 1000);
...
// Description:	Creates an HMM based on a sequence of clusters and the necessary parameters, calculate the probability of the given data sets and writes them into a collective file for later evaluation. Used for call of single threads while multithreading to save computation time.
...
// Description:	Calculates the probability of whether any given tracker (set via currentMovement externally) followed the same movement as it did in the HMM given as input
...
// Description:	Outputs total amount of files present in folder that correspond to the naming convention when used with the input parameters:	trainingFilePath is the path as string to the files to be read trainingFileName is the base name of the files to be read
